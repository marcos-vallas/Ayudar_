...
#define REPORTING_PERIOD_MS     500   // periodo de consulta de valores
...
void setup() 
{
    ...
    Serial.begin(9600); 
   // Serial.begin(115200);  // BaudRate del max30100
    Serial.print("Inicializando pulse oximeter..");
    BTserial.print("Inicializando pulse oximeter..");
...
{   
  //---------------------------Lectura del Max30100 y promedio de valores de bpm--------------------------------------------//
    ...
    pox.update();
    int muestrasCardio=20;  // cantidad de muestras a tomar por el oxímetro  20 muestras / 500 ms = 10 segundos de muestreo, 
 ...
 */
   ...
   tempLectura = map(tempLectura , 0, 640,  0,  50);  //el 600 tiene qe ser 512 pero me da temperatura muy alta
 ...
 float promedioTemp;
  float voltajeTemp;
      ...
      if (indexTemp >= muestrasTemp) { //verifica si ya tomó X muestras de temperatura
        ...
        voltajeTemp = (sumaTemp / muestrasTemp) ; //variable temporal
